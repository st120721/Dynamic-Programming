...
class DP:
    ...
    def optimal_bellman_operator(self, v, cost_to_go):
        ...
        for state in self.all_states:
            ...
            min_value = min(costs)
            action = [allowed_actions[i] for i in range(len(allowed_actions)) if
                      costs[i] == min_value]
            value_function.append(min_value)
    ...
    def plot(self, v, policy):
        ...
        for state, value, action in zip(self.all_states, v, policy):
            ...
            if len(action) > 1:
                for u in action:
                    x, y = self.agent.get_around_states(state)[u][0]
                    plt.arrow(j, i, (y - j) * 0.25, (x - i) * 0.25, head_width=0.2, fc="k")
            elif action[0]:
                x, y = self.agent.get_around_states(state)[action[0]][0]
                plt.arrow(j, i, (y - j) * 0.25, (x - i) * 0.25, head_width=0.2, fc="k")
            else:
                plt.plot(j, i, ".", c="k")
...
c = dp.functions_of_cost_to_go[1]
v1, a2 = dp.policy_iteration(c)
